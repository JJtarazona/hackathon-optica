...
import { createTurno, updateTurno } from "../../Redux/action/turnoAction";
...
const TurnoForm = ({
  ...
  useEffect(() => {
    if (turno) {
      ...
      setFormData({
        ...
        estado: turno?.estado || "Pendiente",
        notas: turno?.notas || "",
        opticaId: localStorage.getItem("opticaId"),
      });
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (turno) {
      dispatch(updateTurno(turno.id, turnoData));
    } else {
      dispatch(createTurno(turnoData));
    }
      ...
      <Card className="w-full max-w-lg bg-card shadow-2xl dark:border-slate-700 max-h-[90vh] flex flex-col">
        ...
        <CardContent className="p-6 space-y-4 overflow-y-auto">
          <form id="turno-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="hora" className="text-foreground/90">
                  Hora
                </Label>
                <Input
                  type="time"
            ...
            <div>
              <Label htmlFor="motivo" className="text-foreground/90">
                Motivo de la Cita
              </Label>
              <Input
                id="motivo"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="estado" className="text-foreground/90">
                  Estado
                </Label>
                <Select
                  name="estado"
            ...
            <div>
              <Label htmlFor="notas" className="text-foreground/90">
                Notas Adicionales
              </Label>
              <Input
                as="textarea"
        ...
        <CardFooter className="flex justify-end gap-2 p-4 border-t border-border dark:border-slate-700">
          ...
          >
            ...
            {formData.id ? "Guardar Cambios" : "Agendar Turno"}
          </Button>
        </CardFooter>
      </Card>
    </motion.div>
