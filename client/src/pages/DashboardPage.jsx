...
import { numeroClientes } from "../Redux/action/clientesAction";
  ...
  <Card className="bg-card shadow-lg dark:border-slate-700 col-span-1 md:col-span-2 lg:col-span-1">
    ...
    <CardContent className="flex flex-col items-center justify-center h-64">
      ...
      <p className="text-sm text-muted-foreground mt-2">
        Gráfico de {title.toLowerCase()} (datos de prueba)
      </p>
      <p className="text-xs text-amber-600 dark:text-amber-400 mt-1 flex items-center">
        <AlertTriangle size={14} className="mr-1" /> Implementación de gráficos
        reales próximamente.
      </p>
    </CardContent>
  </Card>
...
const DashboardPage = () => {
  ...
  const clienteNumero = useSelector((state) => state.cliente.clienteNumero);
  const id = localStorage.getItem("opticaId");

  useEffect(() => {
    dispatch(numeroClientes(id));
  }, [dispatch]);
  ...
  const turnosHoy = turnos.filter(
    (t) => formatDate(t.fecha) === formatDate(new Date().toISOString())
  ).length;
  ...
  const recentActivity = [
    ...clientes.slice(0, 1).map((c) => ({
      type: "Nuevo cliente",
      text: `${c.nombre} ${c.apellido} - Registrado.`,
      color: "text-indigo-500 dark:text-indigo-400",
    ...
    ...turnos
      ...
      .map((t) => ({
        type: "Turno",
        text: `${t.clienteNombre} - Confirmado para ${formatTime(t.hora)}.`,
        color: "text-purple-500 dark:text-purple-400",
        date: t.fecha,
      })),
    ...pedidos
      .filter((p) => p.estado === "En Laboratorio")
      .slice(0, 1)
      .map((p) => ({
        type: "Pedido",
        text: `Pedido ${p.id} para ${p.clienteNombre} - Enviado a lab.`,
        color: "text-pink-500 dark:text-pink-400",
        date: p.fechaPedido,
      })),
    ...facturas
      ...
      .map((f) => ({
        type: "Factura",
        text: `Factura ${f.id} para ${f.clienteNombre} - Pagada.`,
        color: "text-green-500 dark:text-green-400",
        date: f.fechaFactura,
      })),
  ]
  ...
  return (
    ...
    >
      ...
      >
        <ChartPlaceholder
          title="Pacientes Atendidos (Últ. 7 días)"
          icon={Users}
          type="bar"
        />
        <ChartPlaceholder
        ...
        />
        <ChartPlaceholder
          title="Ingresos Mensuales (Últ. 6 meses)"
          icon={DollarSign}
          type="line"
        />
      </motion.div>
            ...
            {recentActivity.length > 0 ? (
              <ul className="space-y-3">
                {recentActivity.map((activity, index) => (
                  <li
                    key={index}
                    className="text-sm text-foreground/90 dark:text-slate-300 flex items-start"
                  >
                    <span className={`font-semibold ${activity.color} mr-1.5`}>
                      {activity.type}:
                    </span>
