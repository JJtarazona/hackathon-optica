...
import { getTurnoByOpticaId } from "../Redux/action/turnoAction";
import { clientePorOptica } from "../Redux/action/clientesAction";
import { useOrganization } from "@clerk/clerk-react";

const TurnosPage = () => {
  const dispatch = useDispatch();
  const [updateTurnos, updateTurnosSet] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [currentTurno, setCurrentTurno] = useState(null);
  const [currentDate, setCurrentDate] = useState(new Date());
  const [preselectedClienteIdForForm, setPreselectedClienteIdForForm] =
    useState(null);

  const turnos = useSelector((state) => state.turno.turno);
  const clientes = useSelector((state) => state.cliente.cliente);
  const { organization } = useOrganization();
  const id = organization?.id;

  console.log("Datos de", turnos);

  useEffect(() => {
    dispatch(getTurnoByOpticaId(id));
  ...
  const handleSubmit = useCallback(
    (formData) => {
      if (currentTurno?.id) {
        updateTurnosSet((prevTurnos) =>
          prevTurnos.map((t) =>
            t.id === currentTurno.id ? { ...formData, id: currentTurno.id } : t
          )
        );
        toast({
          title: "Turno actualizado",
          description: "La cita ha sido modificada.",
        });
      } else {
        updateTurnosSet((prevTurnos) => [
          { ...formData, id: `cita${Date.now()}` },
          ...prevTurnos,
        ]);
        toast({
          title: "Turno agendado",
          description: "Nueva cita registrada exitosamente.",
        });
      }
      resetForm();
    },
  ...
  const handleDelete = useCallback(
    (id) => {
      updateTurnosSet((prevTurnos) => prevTurnos.filter((t) => t.id !== id));
      toast({
          ...
          {filteredTurnos.length > 0 ? (
              ...
              <AnimatePresence>
                {filteredTurnos.map((turno) => (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                      <div className="mb-2 sm:mb-0">
                        <p className="font-semibold text-primary">
                          {formatTime(turno.hora)} - {turno.motivo}
                        </p>
                        ...
                        {turno.optometrista && (
                          <p className="text-xs text-muted-foreground/80 flex items-center mt-0.5">
                            <User className="h-3 w-3 mr-1.5 text-purple-400" />{" "}
                            Opt: {turno.optometrista}
                          </p>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge
                          className={`text-xs ${getStatusColor(turno.estado)}`}
                        >
                          {turno.estado}
                        </Badge>
                        <Button
                    ...
                    {turno.notas && (
                      <p className="text-xs text-muted-foreground mt-1.5 pt-1.5 border-t border-dashed border-border dark:border-slate-700/50">
                        {turno.notas}
                      </p>
                    )}
                  </motion.li>
            ...
            <div className="p-6 text-center">
              <EmptyState
                ...
                description={
                  searchTerm
                    ? "Intente modificar la búsqueda o revise otro día."
