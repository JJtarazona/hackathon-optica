...
import { Home, Users, Calendar, Archive, Package, FileText, Truck, ShoppingCart, Users2, BarChart3, Settings, ChevronsLeft, ChevronsRight, Menu, LogOut } from 'lucide-react';
  ...
  <NavLink
    ...
    className={({ isActive }) =>
      cn(
        ...
        isActive ? "bg-primary text-primary-foreground shadow-lg" : "text-indigo-500 dark:text-slate-300 hover:text-white dark:hover:text-slate-100",
        collapsed ? "justify-center" : ""
      )
    }
  >
    <item.icon className={cn("h-5 w-5 transition-transform duration-200 group-hover:scale-110", collapsed ? "" : "mr-3")} />
    <AnimatePresence>
      {!collapsed && (
        <motion.span
...
const MainLayout = () => {
  ...
  return (
    <div className="flex h-screen text-foreground  transition-colors duration-300">
      ...
      >
        <div className={cn("flex items-center p-4 border-b border-border dark:border-slate-700", sidebarCollapsed ? "justify-center" : "justify-between")}>
          {!sidebarCollapsed && (
            <motion.h1 
          ...
          <Button variant="ghost" size="icon" onClick={toggleSidebar} className="text-muted-foreground hover:text-foreground hover:bg-accent">
            {sidebarCollapsed ? <ChevronsRight size={20} /> : <ChevronsLeft size={20} />}
          </Button>
        </div>
        <nav className="flex-1 p-2 space-y-1.5 ">
          {navItems.map(item => <SidebarLink key={item.to} item={item} collapsed={sidebarCollapsed} />)}
        </nav>
      </motion.aside>
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-border dark:border-slate-700">
              <h1 className="text-2xl font-bold text-primary dark:text-indigo-400 cursor-pointer" onClick={() => { navigate('/app/dashboard'); toggleMobileMenu();}}>OptiCRM</h1>
              <Button variant="ghost" size="icon" onClick={toggleMobileMenu} className="text-muted-foreground hover:text-foreground hover:bg-accent">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-card dark:bg-slate-800 shadow-md p-3 sm:p-4 flex items-center justify-between border-b border-border dark:border-slate-700">
          <Button variant="ghost" size="icon" onClick={toggleMobileMenu} className="md:hidden text-muted-foreground hover:text-foreground hover:bg-accent mr-2">
            <Menu size={24} />
          </Button>
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            <ThemeToggle />
            <Avatar>
              <AvatarImage src="https://avatar.vercel.sh/admin.png?size=40" alt="Admin User" />
